	...
	public function upload ($lang='english')
	{
		...
		if ($this->upload->do_upload()) // Success
		{
			...
			// $result['file_name']	= $conf['img_path'] . '/' . $result['file_name'];
                        $result['file_name']	= $GLOBALS['tinymce_base'] . '/' . $result['file_name'];
			$result['pathnya']	= $config['upload_path'];
			// Output to user
        ...
        public function delete($nama_file){
            $nama_file_send = urldecode($nama_file);
            $config['upload_path']		= $this->config->item('upload_path',	'uploader_settings');
            unlink($config['upload_path'] . "/" . $nama_file_send);
            echo "DONE PROCESS.";
        }
